...
class VideoCamera(object):
    ...
    def get_frame(self):
        ...
        if not success:
            return None, self.acumulado_palabras  # Devolver también la versión acumulada de las palabras
        ...
        if self.sentence and len(self.sentence) > 0 and self.sentence[0] != self.palabras:
            self.palabras += self.sentence[0] + " "  # Concatenar con espacio
            self.acumulado_palabras = self.palabras  # Actualizar la versión acumulada
            self.sentence.pop(0)  # Eliminar la palabra usada de self.sentence
            
        draw_keypoints(image, results)
        ret, jpeg = cv2.imencode('.jpg', image)
        
        return jpeg.tobytes(), self.acumulado_palabras  # Devolver el frame y la versión acumulada
